...
function saveTasks() {
  localStorage.setItem('todo_advanced_tasks', JSON.stringify(tasks));
}
...
function attachEvents() {
  ...
  searchInput.addEventListener('input', e => {
    filteredSearch = e.target.value.toLowerCase();
    renderTasks();
  });
...
function translateUI() {
  ...
  dashboardBtn.textContent = t.dashboardTitle || 'Dashboard';
  backButton.textContent = t.backToList || 'Back to To-Do List';
}
...
function renderTasks() {
  ...
  const t = translations[currentLang];
  let filteredTasks = filteredSearch ? tasks.filter(task => task.text.toLowerCase().includes(filteredSearch)) : tasks;
